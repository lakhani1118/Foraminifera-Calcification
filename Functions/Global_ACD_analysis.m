function [Depthset,R2,RMS,ad,cleanLatitude,cleanLongitude]=Global_ACD_analysis(dat, latitude, longitude,d18O,TimeConstraint,val)
...
for j=1:length(Depthset)
    ...
    for i=1:length(cleanLatitude)
        ...
        if data ==data2
            ...
            ranges(i)=range-1;
            expected=[expected mean(nearland)];
            actual=[actual cleand18O(i)];
            ilist=[ilist i];
        end
    end
    ...
    RMS_val=sqrt(sum((expected(good)-actual(good)).^2)./length(actual(good)));
    if RMS_val<min(RMS)
        optact=actual;
        optexp=expected;
    end
